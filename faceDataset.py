...
class FilteringFaceDataset(IterableDataset):
    
    IMG_EXT = ('.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.tiff', '.webp')

    def __init__(self, path, greyscale=False, ext=".jpg", size=None, min_size:int = 64):
        """
        This is a docstring
        """
        super(FilteringFaceDataset, self).__init__()
    ...
    def __next__(self):
        ...
        p = self.fileList[self.curr_pos]
        raw_image = torchvision.io.read_image(p, self.readMode)

        if raw_image.shape[1] < self.min_size:
            
            return next(self)

        else:

            p = self.rescaler(raw_image.type(torch.FloatTensor)) / 255.0

            if self.alpha > 0:

                y = self.double_rescaler(raw_image.type(torch.FloatTensor)) / 255.0
                y = self.upsampler(y)
    ...
    def set_scale(self, scale):
        ...
        """
        self.scale = scale
        self.double_rescaler = self.rescaler
        self.rescaler = torchvision.transforms.Resize((self.scale, self.scale))
